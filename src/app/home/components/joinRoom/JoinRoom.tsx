...
const JoinRoom = () => {
    const { hasMore, isFetching, isLoading, onNextPage, onJoin, rooms, searchText, setSearchText } = useJoinRoomViewModel();
        ...
        <main className="bg-background h-full w-full py-md md:rounded-xl flex gap-md sm:p-md flex-col overflow-hidden">
            ...
            <div className="h-full flex-1 flex flex-col gap-md overflow-y-auto">
                ...
                {!isLoading && <>
                    ...
                    {isFetching ? <h1 className="text-body text-input-placeholder text-center">Loading ....</h1> : hasMore && <h1 className="text-body text-input-placeholder cursor-pointer text-center" onClick={()=>onNextPage()}>Load more ....</h1>}
                    {rooms.length === 0 &&
